...
export const adminCustomerService = {
    getAllUsers: async (page: number = 1, limit: number = 10, search: string = '', status: string = '', sortBy: string = '') => {
        const response = await axiosInstance.get(`${API_URL}?page=${page}&limit=${limit}&search=${search}&status=${status}&sortBy=${sortBy}`);
